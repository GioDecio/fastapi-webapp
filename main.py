...
# TODO" Remove cast: # cast() is a no-op at runtime; it only tells mypy the concrete type of SQLAlchemy's Any-typed results
...
def home(request: Request, db: Annotated[Session, Depends(get_db)]) -> Any:
...
def create_user(
    user: UserCreate,
    db: Annotated[Session, Depends(get_db)],  # Each request gets its db session
) -> UserResponse:  # Automatically parses json body, validates schema and return error before the function is run
    result = db.execute(
...
def update_post_partial(
    ...
    # exclude_uset=True Pydantic will not include fields that are not updated, so we get what the client actually sent
