...
async def create_user(user: UserCreate, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    db.add(new_user)  # no IO yet, so no need to use async
    await db.commit()
...
@router.patch(
    "/me/password", status_code=status.HTTP_200_OK
)  # No separate auth check is needed here,we know who the user is from the token
...
async def delete_user(
    ...
    await db.delete(user)  # interacts with the session, so async is needed
    await db.commit()
...
async def upload_profile_picture(
    ...
    # process_profile_image is sync (Pillow), so run it in a thread to avoid blocking the event loop
    # UnidentifiedImageError is raised if the uploaded bytes are not a valid image
    try:
...
async def delete_user_picture(
    ...
    # commit first so the file is only deleted if the db update succeeds
